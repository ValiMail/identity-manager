...
class Bootstrap:
    ...
    def public_identity_is_valid(self):
        ...
        dns_override = os.getenv("DNS_OVERRIDE", None)
        try:
            dns_tlsa_rr = DANE.get_first_leaf_certificate(id_name, dns_override)
        except (dns.resolver.NoAnswer, TLSAError) as err:
            return False, str(err)
        if dns_tlsa_rr is None:
            err = "No TLSA entity record found for {}".format(id_name)
            return False, err
    ...
    def cert_matches_private_key(self, cert_obj):
        ...
        identity = Identity(self.identity_name, private_key=self.get_pki_asset("key"))
        return identity.cert_matches_private_key(cert_obj)
